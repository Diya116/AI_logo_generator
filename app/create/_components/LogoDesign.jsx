      ...
      <div className='grid grid-cols-2 md:grid-cols-3 gap-10 mt-5'>
       {
        LogoDesignData.map((design,index)=>(
          <div key={index} className={`p-2 cursor-pointer hover:border-2 border-primary rounded-md ${selectedOption===design.title&&'border-primary border-2 rounded-md'}`} onClick={()=>{setSelectedOption(design.title);onHandleInputChange(design.title)}}>
          <Image src={design.image} alt={design.title} width={300} height={200} className='w-full rounded-xl object-cover h-[150px]'/>
